...
min_orf_size = 3000  # when 3000, 1000 aa
start_codon = 'ATG'
...
f='Paramyxoviridae_220803_nucl.fa'
fa=parse_fasta(f)
...
out=['fasta_header\tseq_length\tis_rule_of_6\tstarts_with_ACC..GGT\tpromoter_length\tpromoter_leader\tpromoter_trailer\talignment_leader\talignment_trailer\talignment\tnum_ORF_longer_than_1000aa\tend_of_last_ORF(0-based)\t3UTR_length\tnum_NNNNNC_found\tNNNNNC_positions\tnum_CGNNNN_found\tCGNNNN_positions\tseq[-96:-72]\t%s\t%s\n' % (c_pos_header, cg_pos_header)]
for h in fa:
    if not 'complete genome' in h:
        continue
    seq = fa[h]
    ...
    is_ACC = test_ACC(seq)
    orf_pos, L_end = detect_orf(seq)
    NNNNNC = find_NNNNNC(seq[-motif_search_len:])
    CGNNNN = find_CGNNNN(seq[-motif_search_len:])
    if L_end == 'NA':
        UTR_length = 'NA'
    else:
        UTR_length = len(seq) - int(L_end)
    out.append('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % (
        ...
        UTR_length,
        len(NNNNNC),
        ';'.join([ str(pos) for pos in NNNNNC ]),
        len(CGNNNN),
        ';'.join([ str(pos) for pos in CGNNNN ]),
        motif_formatter2(seq, -96, -72),
        motif_formatter(seq, NNNNNC, c_pos, 5),
        motif_formatter(seq, CGNNNN, cg_pos, 0),
    ))

with open('result_220818.wide.tsv', 'w') as outfile:
